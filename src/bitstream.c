...
int bit_stream_flush(bit_stream_t* stream, int add_pad) {
    ...
    assert(fwrite(stream->buffer, sizeof(byte_t), num_bytes_to_write, stream->file) == num_bytes_to_write &&
           "bit_stream: cannot flush");
...
int bit_stream_write(bit_stream_t* stream, byte_t* data, int bit_length) {
    while (bit_length > 0) {
...
// int bit_stream_read(bit_stream_t* stream, byte_t* data, int bit_length);
